-- Production scheduling and front-desk state machine.
...
create index appointments_practitioner_time_idx
  on clinical_core.appointments(practitioner_person_id, starts_at)
...
revoke all on clinical_core.appointments, clinical_core.appointment_status_events
  from public, clinical_core_api;
create trigger appointment_status_events_append_only
  before update or delete on clinical_core.appointment_status_events
...
alter table clinical_audit.events add constraint events_action_check check (action in (
  ...
  'appointment.booked','appointment.rescheduled','appointment.status_changed',
...
alter table clinical_audit.events add constraint events_resource_type_check check (resource_type in (
  ...
  'report','audit_log','organization_membership','review_queue_item','appointment'));
...
begin
  ...
  select coalesce(jsonb_agg(row_data order by starts_at,id),'[]'::jsonb) into _appointments from (
    ...
    left join clinical_core.patient_records patient on patient.id=appointment.patient_record_id
      and patient.organization_id=_organization_id and patient.deleted_at is null
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'user_id',membership.person_id,'display_name',case when membership.person_id=clinical_private.actor_person_id() then 'You' else 'Practitioner' end,
  ...
  select coalesce(jsonb_agg(jsonb_build_object('id',patient.id,'name',
    coalesce(nullif(btrim(concat_ws(' ',patient.first_name,patient.last_name)),''),'Unnamed patient'))
    order by patient.last_name,patient.first_name,patient.id),'[]'::jsonb) into _patients
    from clinical_core.patient_records patient where patient.organization_id=_organization_id
      and patient.deleted_at is null and patient.status='active';
...
begin
  ...
  if _patient_id is not null and not exists(select 1 from clinical_core.patient_records patient
    where patient.id=_patient_id and patient.organization_id=_organization_id and patient.deleted_at is null and patient.status='active') then
  ...
  if exists(select 1 from clinical_core.appointments appointment where appointment.organization_id=_organization_id
    and appointment.practitioner_person_id=_practitioner_user_id
    ...
    or (_patient_id is not null and exists(select 1 from clinical_core.appointments appointment where appointment.organization_id=_organization_id
      and appointment.patient_record_id=_patient_id
      and appointment.deleted_at is null and appointment.status not in ('cancelled','no_show')
  ...
  values(_organization_id,clinical_private.actor_person_id(),'appointment.booked','appointment',_id,_patient_id,
    'Appointment booked','clinical_data',jsonb_build_object('appointment_type',_appointment_type,'location_present',nullif(btrim(_location),'') is not null,'telehealth',nullif(btrim(_telehealth_url),'') is not null));
...
begin
  ...
  if _idempotency_key is not null then select * into _replay from clinical_core.appointment_status_events
    ...
    if found then return jsonb_build_object('ok',true,'id',_appointment.id,'status',_replay.to_status,
      'previous_status',_replay.from_status,'version',_appointment.version,'already_applied',true); end if; end if;
  ...
  update clinical_core.appointments set status=_to_status,version=version+1,updated_by_person_id=clinical_private.actor_person_id(),updated_at=clock_timestamp() where id=_appointment.id;
  ...
  values(_appointment.organization_id,clinical_private.actor_person_id(),'appointment.status_changed','appointment',_appointment.id,_appointment.patient_record_id,
    'Appointment status changed','clinical_data',jsonb_build_object('previous_status',_appointment.status,'status',_to_status,'reason_present',coalesce(char_length(btrim(_reason)),0)>0));
  return jsonb_build_object('ok',true,'id',_appointment.id,'status',_to_status,'previous_status',_appointment.status,'version',_appointment.version+1,'already_applied',false);
end $$;

create or replace function clinical_core.update_appointment_status(_appointment_id uuid,_status text)
returns jsonb language sql security invoker set search_path='' as $$
  select jsonb_build_object('id',result->>'id','status',result->>'status','previous_status',result->>'previous_status','already_set',(result->>'already_applied')::boolean)
...
begin
  ...
  select * into _appointment from clinical_core.appointments where id=_appointment_id and organization_id=clinical_private.organization_id() and deleted_at is null for update;
  if not found then raise exception using errcode='P0002',message='appointment_not_found'; end if;
  if not clinical_private.has_schedule_role(_appointment.organization_id) then raise exception using errcode='42501',message='schedule_role_required'; end if;
  ...
  if exists(select 1 from clinical_core.appointments other where other.organization_id=_appointment.organization_id
    ...
    and other.deleted_at is null and other.status not in ('cancelled','no_show') and tstzrange(other.starts_at,other.ends_at,'[)')&&tstzrange(_starts_at,_ends_at,'[)')) then raise exception using errcode='22023',message='appointment_overlap'; end if;
  if _appointment.patient_record_id is not null and exists(select 1 from clinical_core.appointments other
    where other.organization_id=_appointment.organization_id and other.patient_record_id=_appointment.patient_record_id
      ...
      and tstzrange(other.starts_at,other.ends_at,'[)')&&tstzrange(_starts_at,_ends_at,'[)')) then raise exception using errcode='22023',message='appointment_overlap'; end if;
  update clinical_core.appointments set starts_at=_starts_at,ends_at=_ends_at,updated_by_person_id=clinical_private.actor_person_id(),updated_at=clock_timestamp() where id=_appointment.id;
  insert into clinical_audit.events(organization_id,actor_person_id,action,resource_type,resource_id,patient_record_id,safe_message,purpose,safe_metadata)
  values(_appointment.organization_id,clinical_private.actor_person_id(),'appointment.rescheduled','appointment',_appointment.id,_appointment.patient_record_id,'Appointment rescheduled','clinical_data',jsonb_build_object('time_changed',true));
  return jsonb_build_object('id',_appointment.id,'status',_appointment.status,'starts_at',_starts_at,'ends_at',_ends_at);
end $$;

create or replace function clinical_core.correct_appointment_status(_appointment_id uuid,_to_status text,_reason text,_expected_version integer)
returns jsonb language plpgsql security definer set search_path='' as $$
declare _appointment clinical_core.appointments%rowtype;
begin
  perform clinical_private.assert_production_context(clinical_private.organization_id(),'clinical_data','workforce');
  if _to_status not in ('scheduled','confirmed','arrived','in_encounter','completed','cancelled','no_show') or nullif(btrim(_reason),'') is null or char_length(_reason)>1000 then raise exception using errcode='22023',message='correction_invalid'; end if;
  select * into _appointment from clinical_core.appointments where id=_appointment_id and organization_id=clinical_private.organization_id() and deleted_at is null for update;
  if not found then raise exception using errcode='P0002',message='appointment_not_found'; end if;
  if not exists(select 1 from clinical_core.organization_memberships caller where caller.organization_id=_appointment.organization_id and caller.person_id=clinical_private.actor_person_id() and caller.status='active' and caller.role in ('owner','admin')) then raise exception using errcode='42501',message='organization_admin_required'; end if;
  if _expected_version is not null and _expected_version<>_appointment.version then raise exception using errcode='40001',message='appointment_version_conflict'; end if;
  if _appointment.status not in ('completed','cancelled','no_show') or _appointment.status=_to_status then raise exception using errcode='22023',message='correction_refused'; end if;
  update clinical_core.appointments set status=_to_status,version=version+1,updated_by_person_id=clinical_private.actor_person_id(),updated_at=clock_timestamp() where id=_appointment.id;
  insert into clinical_core.appointment_status_events(organization_id,appointment_id,from_status,to_status,kind,reason,actor_person_id)
  values(_appointment.organization_id,_appointment.id,_appointment.status,_to_status,'correction',btrim(_reason),clinical_private.actor_person_id());
  insert into clinical_audit.events(organization_id,actor_person_id,action,resource_type,resource_id,patient_record_id,safe_message,purpose,safe_metadata)
  values(_appointment.organization_id,clinical_private.actor_person_id(),'appointment.corrected','appointment',_appointment.id,_appointment.patient_record_id,'Appointment status corrected','clinical_data',jsonb_build_object('previous_status',_appointment.status,'status',_to_status));
  return jsonb_build_object('ok',true,'id',_appointment.id,'status',_to_status,'previous_status',_appointment.status,'version',_appointment.version+1,'already_applied',false);
end $$;
...
revoke all on function clinical_core.get_desktop_calendar(uuid,timestamptz,timestamptz),clinical_core.book_appointment(uuid,uuid,text,timestamptz,timestamptz,uuid,text,text,text),clinical_core.transition_appointment(uuid,text,integer,text,text),clinical_core.update_appointment_status(uuid,text),clinical_core.reschedule_appointment(uuid,timestamptz,timestamptz),clinical_core.correct_appointment_status(uuid,text,text,integer) from public;
grant execute on function clinical_core.get_desktop_calendar(uuid,timestamptz,timestamptz),clinical_core.book_appointment(uuid,uuid,text,timestamptz,timestamptz,uuid,text,text,text),clinical_core.transition_appointment(uuid,text,integer,text,text),clinical_core.update_appointment_status(uuid,text),clinical_core.reschedule_appointment(uuid,timestamptz,timestamptz),clinical_core.correct_appointment_status(uuid,text,text,integer) to clinical_core_api;
