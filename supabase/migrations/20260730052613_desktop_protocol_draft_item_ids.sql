-- desktop_protocol_draft_item_ids
--
-- save_protocol_draft now returns `itemIds` — the ids of the rows it just
-- wrote, in payload order.
--
-- Why this matters and is not cosmetic: an autosave replaces the draft's items
-- wholesale, so every save mints new row ids. Without them the client cannot
-- address the item it just persisted, and the practitioner's interaction review
-- (review_protocol_item_interactions, which takes a persisted item id) has
-- nothing to point at. The alternative — letting the UI guess, or refetching
-- and risking a clobber of in-flight edits — is worse.
--
-- Identical to 20260730050009 in every other respect.
