-- Phase 10A follow-up: input_sha256 / output_sha256 CHECK constraints on
-- the pre-existing table require non-empty strings (length=64). Seed with
-- a placeholder 64-char digest so a newly-created run is legal; finalize
-- overwrites both when the run completes.
...
begin
  ...
  values
    (_organization_id, _patient_id, _encounter_id, _pathway,
     ...
     _placeholder_sha, 'pre', _rule_set_version, _prompt_version,
