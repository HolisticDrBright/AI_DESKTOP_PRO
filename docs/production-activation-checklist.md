...
- Production Aurora contains the reviewed portable empty clinical core: 17 application tables, eight migration-ledger entries, and zero clinical/audit rows. A separate deployed JWT API has no clinical data-plane permissions and always fails closed. Seven patient/lab/audit operations have an activation-gated source implementation, but that functional candidate is not deployed; all 222 inventory entries are disabled in the deployed route and 215 still need reviewed AWS implementations for full Desktop functionality.
- Exact candidate source `c36648dfa237f60a204df471711dfc6c103c6882` now implements all 21 governed App/Desktop JWT routes against production-specific consent, lab, clinical-record, privacy, and Desktop adapters. It requires both correct Cognito pools, `custom:production_bound=true`, no synthetic attestation, `PHI_ALLOWED=true`, approved activation state, and a 64-character evidence hash. The deployed stack remains `PhiAllowed=false`, `ActivationState=blocked`, and `DataPlaneEnabled=false`, so every functional route returns bounded 503 and cannot access Aurora.
- Both production Cognito pools now contain the immutable `custom:production_bound` schema attribute, but no production users or approved identity bindings exist. The activation-ready CloudFormation candidate and guarded PHI-disabled replacement script are implemented and the exact closed deployment passed evidence exercise v2 (`b425616777af158967b80c62b43aee85b21be94ef1e4310dabe19f6cbbb8cc54`). The role has one log-only policy and no attached or clinical data-plane permissions. This is engineering preparation, not approval.
