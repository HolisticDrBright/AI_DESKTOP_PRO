...
- Production Aurora contains the reviewed portable empty clinical core: 17 application tables, nine migration-ledger entries, and zero clinical/audit rows. A separate deployed JWT API has no clinical data-plane permissions and always fails closed. Eleven patient/lab/audit/workforce operations have an activation-gated source implementation, but all 222 inventory entries remain disabled in the deployed route and 211 still need reviewed AWS implementations for full Desktop functionality. Legacy workforce add-by-email and self-activation remain refused pending governed Cognito identity binding.
- Exact candidate source `c4d309f53046f9dd45092eae4868a54cc1b51e32` now implements all 21 governed App/Desktop JWT routes against production-specific consent, lab, clinical-record, privacy, and Desktop adapters. It requires both correct Cognito pools, `custom:production_bound=true`, no synthetic attestation, `PHI_ALLOWED=true`, approved activation state, and a 64-character evidence hash. The deployed stack remains `PhiAllowed=false`, `ActivationState=blocked`, and `DataPlaneEnabled=false`, so every functional route returns bounded 503 and cannot access Aurora.
- Both production Cognito pools now contain the immutable `custom:production_bound` schema attribute, but no production users or approved identity bindings exist. The activation-ready CloudFormation candidate and guarded PHI-disabled replacement script are implemented and the exact closed deployment passed evidence exercise v2 (`2b1868fadcbad96c2144dc35a7feb40453d6c96f9474a3c51b25dd3ed35886fd`). The role has one log-only policy and no attached or clinical data-plane permissions. This is engineering preparation, not approval.
...
- Aurora point-in-time recovery restored the prior empty seven-migration schema into a private encrypted temporary cluster and verified 17 tables and zero patient/audit rows. The exact temporary recovery resources were deleted afterward; the production cluster remains encrypted, deletion-protected, and configured for 35-day backup retention. The eighth audit and ninth workforce-membership migrations require a fresh recovery exercise before activation. Snapshot/object/Cognito/application rollback and incident tabletop items remain open.
