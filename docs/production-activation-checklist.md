...
- Production Aurora contains the reviewed portable empty clinical core: 30 application tables, 16 migration-ledger entries, and zero clinical/audit/task/appointment/encounter/note/protocol rows. A separate deployed JWT API has no clinical data-plane permissions and always fails closed. Forty-six patient/lab/audit/workforce/review-queue/scheduling/encounter/note/overview/sync-control/protocol operations have an activation-gated source implementation, but all 222 inventory entries remain disabled in the deployed route and 176 still need reviewed AWS implementations for full Desktop functionality. Legacy workforce add-by-email and self-activation remain refused pending governed Cognito identity binding.
