...
- One exact-commit Desktop readiness image is in production ECR and scans at 0 Critical/0 High. One private ECS task is RUNNING/HEALTHY with no public ingress, secrets, or application IAM permissions. It permits only health checks and returns 503 for all normal traffic; `AWS_CLINICAL_ADAPTER_READY=false` and `PHI_ALLOWED=false`.
