...
- The undeployed functional candidate now implements all 21 governed App/Desktop JWT routes against production-specific consent, lab, clinical-record, privacy, and Desktop adapters. It requires both correct Cognito pools, `custom:production_bound=true`, no synthetic attestation, `PHI_ALLOWED=true`, approved activation state, and a 64-character evidence hash. None of those routes are deployed with data-plane permissions yet.
- The activation-ready CloudFormation candidate and guarded PHI-disabled replacement script are implemented. The template defaults closed and conditionally omits Aurora/secret/KMS data permissions unless all independent activation inputs are coherent. Its existence is engineering preparation, not approval; the currently deployed stack remains log-only until an exact closed candidate deployment is separately verified.
