...
- Production Aurora contains the reviewed portable empty clinical core: 20 application tables, 11 migration-ledger entries, and zero clinical/audit/task/appointment rows. A separate deployed JWT API has no clinical data-plane permissions and always fails closed. Twenty patient/lab/audit/workforce/review-queue/scheduling operations have an activation-gated source implementation, but all 222 inventory entries remain disabled in the deployed route and 202 still need reviewed AWS implementations for full Desktop functionality. Legacy workforce add-by-email and self-activation remain refused pending governed Cognito identity binding.
- Exact candidate source `c662ffcf844cce89356f809c38cdeb4a8cc1d457` now implements all 21 governed App/Desktop JWT routes against production-specific consent, lab, clinical-record, privacy, and Desktop adapters. It requires both correct Cognito pools, `custom:production_bound=true`, no synthetic attestation, `PHI_ALLOWED=true`, approved activation state, and a 64-character evidence hash. The deployed stack remains `PhiAllowed=false`, `ActivationState=blocked`, and `DataPlaneEnabled=false`, so every functional route returns bounded 503 and cannot access Aurora.
- Both production Cognito pools now contain the immutable `custom:production_bound` schema attribute, but no production users or approved identity bindings exist. The activation-ready CloudFormation candidate and guarded PHI-disabled replacement script are implemented and the exact closed deployment passed evidence exercise v2 (`8e47401874e1f3de6cb3c5f2c7813a3ef040da2b1a0fdb2eda982f44ade6b3ad`). The role has one log-only policy and no attached or clinical data-plane permissions. This is engineering preparation, not approval.
...
- Aurora point-in-time recovery has now restored the current empty 11-migration schema into a private encrypted full-copy cluster and independently verified 20 tables and zero clinical/audit/appointment rows in 755.1 seconds. The exact temporary writer and cluster were deleted afterward; the production cluster remains encrypted, deletion-protected, and configured for 35-day backup retention. Evidence SHA-256: `98a3183c5f84ef1e5014e3c595cc8b05719064b87aed18295c25e7f9c3a3ed63`. Snapshot/object/Cognito/application rollback and incident tabletop items remain open.
