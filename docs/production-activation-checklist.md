...
- Production Aurora contains the reviewed portable empty clinical core: 26 application tables, 15 migration-ledger entries, and zero clinical/audit/task/appointment/encounter/note rows. A separate deployed JWT API has no clinical data-plane permissions and always fails closed. Thirty-nine patient/lab/audit/workforce/review-queue/scheduling/encounter/note/overview/sync-control operations have an activation-gated source implementation, but all 222 inventory entries remain disabled in the deployed route and 183 still need reviewed AWS implementations for full Desktop functionality. Legacy workforce add-by-email and self-activation remain refused pending governed Cognito identity binding.
...
- Both production Cognito pools now contain the immutable `custom:production_bound` schema attribute, but no production users or approved identity bindings exist. The activation-ready CloudFormation candidate and guarded PHI-disabled replacement script are implemented and the exact closed deployment passed evidence exercise v2 (`2b1eff659d9d9469f21c4f07ef995037c75ff73d685a0acc7e2e0f05185fe078`). The role has one log-only policy and no attached or clinical data-plane permissions. This is engineering preparation, not approval.
...
- Aurora point-in-time recovery restored the current empty 15-migration schema into a private encrypted full-copy cluster and independently verified 26 tables and zero clinical/audit rows in 691.1 seconds. The exact temporary writer and cluster were deleted afterward; the production cluster remains encrypted, deletion-protected, and configured for 35-day backup retention. Evidence SHA-256: `ebbe679d77f940499fd99c5fdf78cbd8fe9d3458bd0d47bc0e5e34d109afed3f`. Snapshot/object/Cognito/application rollback and incident tabletop items remain open.
...
- A 2026-08-24 source candidate adds four AWS-native patient-protocol tables and seven guarded protocol operations, raising the implementation inventory to 46/222 and reducing the unported inventory to 176. It has not been applied to production: the live database remains at 26 tables/15 migrations and the deployed route still enables zero operations. Product-bearing protocols fail closed at approval until governed production catalog evidence is available; commercial links are refused.
