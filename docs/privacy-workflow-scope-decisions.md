...
| **Cross-store export coverage** (lab processing jobs and documents, voice transcripts) | **implemented, locally verified** (Desktop migration 100 `20260920180000_production_owned_privacy_export_cross_store.sql`; `aws-cross-store-export-reader.ts`) | The prepared copy gains `labs` and `voice` sections read under the owner's own identity from the external stores where the deployment names them (table, bucket, key and a review digest per store, on top of export delivery); the coverage statement on every job and in the manifest says which stores were included. Documents are inlined up to 6 MiB each and otherwise listed with the reason. Nothing is configured in any deployed stack; hosted verification has not run. Still excluded: clinic records, identity and billing, device-only data, recovery archives, backups and audit logs. |
