...
Source status is no longer tracked in this document: it went stale within days and a
stale status list is worse than none. The authoritative record is V2
`expo/docs/commercial-six-phase-ledger.md` (one entry per increment, newest first) and
the current handoff `expo/docs/claude-code-handoff-2026-09-20.md`. Nothing in either is
deployed; PHI stays disabled. The eight items below are the engineering that obtaining
contracts does not complete, and they are reviewed against the ledger rather than
restated here.
...
2. **Expanded production feature contracts.** September 19: the Core launch scope is now declared once (`infra/aws-clinical-core/core-launch-scope.json`, gate `check:aws-core-launch-scope`, personal posture route) and V2 follows the deployed allow-list; see [Core launch scope](core-launch-scope.md). Still open: deploy separately approved lab analysis, catalog, Ask ALP, transcription, daily guidance, reproductive-health, telehealth and family routes required by the chosen commercial scope. Several current features run on synthetic services and cannot be made live by removing the banner. Retire synthetic Fly routing from the commercial release.
