...
| Pricing | Set Core, Longevity and Peptide prices, billing period, trials, refunds, and whether add-ons can be combined. Confirm telehealth appointment types, prices, duration, cancellation and no-show policy. | Approved price/policy sheet; no invented prices |
...
3. **Consumer subscription service.** Implement server-verified store receipts/events and/or approved checkout, durable AWS entitlement lookup, renewals, replay protection, refund/revocation, restore purchases and real paywalls. `PROGRAM_ENTITLEMENT_SERVICE_AVAILABLE=false` still exists; the synthetic preview is not a completed billing implementation.
