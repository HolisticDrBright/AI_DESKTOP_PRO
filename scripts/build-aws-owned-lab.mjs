...
const template={AWSTemplateFormatVersion:'2010-09-09',Description:'Independent consumer lab/document processing candidate; default blocked, no seeded approvals',
  ...
  Conditions:{...Object.fromEntries(['ReviewedRanges','ReviewedKnowledge'].map(k=>[k,source.Conditions[k]])),
    ...
    // qualification database with PHI disabled; data permissions ride Enabled (Active or Qualification), never a third state.
...
template.Rules={...qualificationRules({extra:[
  {Assert:{'Fn::Equals':[ref('AllowedScopes'),'ai_context,lab_history']},AssertDescription:'Explicit lab-history and AI scopes required for qualification execution'},
  {Assert:{'Fn::Equals':[ref('LabRangeMode'),'reviewed_release']},AssertDescription:'Qualification classifies documents only against a signed reviewed range release'},
...
function gate(roleName,logsPolicyName,extra=[]){
  ...
  role.Policies=[{PolicyName:'LogsOnly',PolicyDocument:logs.PolicyDocument},
    ...rest.map(p=>p['Fn::If']?{'Fn::If':['Enabled',{'Fn::If':[p['Fn::If'][0],p['Fn::If'][1],ref('AWS::NoValue')]},ref('AWS::NoValue')]}:{'Fn::If':['Enabled',p,ref('AWS::NoValue')]}),
    ...(extra.length?[{'Fn::If':['Enabled',{PolicyName:'OwnedConsentDatabase',PolicyDocument:{Version:'2012-10-17',Statement:extra}},ref('AWS::NoValue')]}]:[])];
}
...
cleanupRole.Policies=[{PolicyName:'LogsOnly',PolicyDocument:{Version:'2012-10-17',Statement:[cleanupLogs]}},
  {'Fn::If':['Enabled',{PolicyName:'ScopedPersonalLabCleanup',PolicyDocument:{Version:'2012-10-17',Statement:[...cleanupStatements.filter(s=>s!==cleanupLogs),...database]}},ref('AWS::NoValue')]}];
for(const name of ['LabCancellationPolicy','LabInventoryQueryPolicy'])R[name].Condition='Enabled';
for(const name of ['LabCleanupSweepRule','LabCleanupObjectRule'])R[name].Properties.State={'Fn::If':['Enabled','ENABLED','DISABLED']};
