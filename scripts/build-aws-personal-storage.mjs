...
['GET records','POST records','GET record','GET consent','POST consent','GET chat-context','POST privacy-export','GET privacy-export'].forEach((route,index)=>{const [method,resource]=route.split(' ');template.Resources[`Route${index}`]={Type:'AWS::ApiGatewayV2::Route',Properties:{ApiId:ref('ApiId'),RouteKey:`${method} /clinical-core/consumer/personal/${resource}`,AuthorizationType:'JWT',AuthorizerId:ref('ConsumerAuthorizerId'),Target:{'Fn::Join':['/',['integrations',ref('Integration')]]}}};});
