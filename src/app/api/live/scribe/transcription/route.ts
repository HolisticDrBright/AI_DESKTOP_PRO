...
import { recordingTranscriptionRequest } from '@/adapters/recording-transcription.server';
...
/** Same-origin proxy for the transcription authority. Identity is only the
 * request-scoped workforce cookie; the client cannot pick a release, actor or
 * organization. Transcript text is returned only for `read` and never logged. */
export async function POST(request: Request) {
  ...
  try {
    ...
    // A correction carries up to 2,000,000 characters of text.
    const body = await readBoundedRequestBody(request, 2_200_000, 20000);
    let input: unknown;
    ...
    return Response.json(await recordingTranscriptionRequest(parsed.data, session.token, request.signal), { headers });
  } catch (error) {
