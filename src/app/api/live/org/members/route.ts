...
/** GET → the active organization's roster (admin-gated by the database RPC). */
...
/** POST { email, role } → link an existing identity to the active organization. */
export async function POST(req: NextRequest) {
  ...
  return runLive(async () => {
    ...
    if (!email.includes("@") || email.length > 320) {
      throw new AdapterError("invalid", "Enter the member's email address.");
    }
    if (!ROLES.includes(role)) {
      throw new AdapterError("invalid", "Choose a role for the member.");
    }
    const session = await getRequestSession();
