...
export async function POST(req: NextRequest) {
  ...
  return runLive(async () => {
    ...
    if (typeof b.flagId !== "string" || !b.flagId) throw new Error("flagId is required");
    if (b.action !== "acknowledge" && b.action !== "override" && b.action !== "resolve") {
      throw new Error("action must be acknowledge, override or resolve");
    }
    ...
    if (b.action === "override" && (typeof b.reason !== "string" || !b.reason.trim())) {
      throw new Error("overriding a safety flag requires a reason");
    }
