...
import { draftPlanFromTemplate } from "@/server/nutrition-copilot";
...
export async function POST(req: NextRequest) {
  ...
  return runLive(async () => {
    ...
    if (!version) throw new Error("record not found");
    ...
    return draftPlanFromTemplate({
      template: {
        ...
        // A plan built from a starter template inherits its review requirement;
        // one authored from scratch still needs review before approval.
        requiresPractitionerReview: true,
        foodRules: content.foodRules.map((r) => ({
          disposition: r.disposition as "emphasize" | "include" | "limit" | "avoid" | "conditional",
          label: r.label,
