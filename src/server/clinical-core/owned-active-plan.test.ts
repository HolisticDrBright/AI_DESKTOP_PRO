...
const pointer=(patch:Record<string,unknown>={})=>({recordId:uuid,revision:2,contentSha256:'a'.repeat(64),consentRevision:1,adoptedAt:'2026-09-16T12:00:00.000Z',adoptionRequestId:request,
...
describe('owned active plan pointer',()=>{
  ...
  it('adopts an exact record revision with an explicit predecessor and idempotent request identity',async()=>{
    ...
    const state=await s.adapter.adoptActivePlan(context,{recordId:uuid,revision:2,contentSha256:'a'.repeat(64),consentRevision:1,requestId:request,expectedPrevious:{recordId:other,revision:1}});
    expect(state.current?.recordId).toBe(uuid);expect(state.duplicate).toBe(false);
    expect(s.query.mock.calls[1][0]).toBe('select clinical_core.adopt_owned_active_plan($1,$2,$3,$4,$5,$6,$7) as result');
    expect(s.query.mock.calls[1][1]).toEqual([clinicalUuid(uuid),2,'a'.repeat(64),1,clinicalUuid(request),clinicalUuid(other),1]);
    const first=setup({current:pointer({supersedes:null}),history,historyLimit:100,duplicate:true});
    expect((await first.adapter.adoptActivePlan(context,{recordId:uuid,revision:2,contentSha256:'a'.repeat(64),consentRevision:1,requestId:request,expectedPrevious:null})).duplicate).toBe(true);
    expect(first.query.mock.calls[1][1]).toEqual([clinicalUuid(uuid),2,'a'.repeat(64),1,clinicalUuid(request),null,null]);
  });
  ...
  it('maps database refusals to storage errors and refuses a pointer the database did not actually set',async()=>{
    ...
    await expect(wrong.adapter.adoptActivePlan(context,{recordId:uuid,revision:2,contentSha256:'a'.repeat(64),consentRevision:1,requestId:request,expectedPrevious:null})).rejects.toMatchObject({code:'storage_unavailable'});
    ...
    expect(stale.query.mock.calls[1][0]).toBe('select clinical_core.release_owned_active_plan($1,$2,$3) as result');
