...
export function labRecoveryDescriptor(row: Record<string, unknown>, scope: Scope, now = Date.now()) {
  ...
  const publication = row.publication as Record<string, unknown> | undefined;
  const publicationStatus = publication && typeof publication === 'object' && ['published','pending','refused'].includes(String(publication.status)) ? String(publication.status) as 'published'|'pending'|'refused' : undefined;
