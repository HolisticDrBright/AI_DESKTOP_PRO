...
function fixture(){
  const a={organizationId:randomUUID(),recordingId:randomUUID(),sessionId:randomUUID(),runId:randomUUID(),storage:{bucket:'fictional-bucket',expectedBucketOwner:'123456789012',region:'us-east-2'},
    inventory:[{objectKey:'exact-key',segmentId:randomUUID()}],transcriptionInventory:[]} as unknown as RecordingCleanupAdmission;
...
describe('AWS recording cleanup adapter',()=>{
  it('pins owner/prefix and qualifies versioning and Object Lock before listing',async()=>{
    ...
    expect(f.send.mock.calls.at(-1)?.[0].input).toEqual({Bucket:'fictional-bucket',ExpectedBucketOwner:'123456789012',
      Prefix:`encounter-recordings/${f.a.organizationId}/${f.a.recordingId}/`,MaxKeys:100});
  });
  ...
  it('requires an attempt and sends only an exact version delete without governance bypass',async()=>{
    ...
    f.a.attempt={id:randomUUID(),segmentId:f.a.inventory[0].segmentId,artifactId:null,objectVersion:f.v.version,kind:'object',evidenceSha256:'a'.repeat(64)};
