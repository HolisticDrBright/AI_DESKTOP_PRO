...
export function createRecordingCaptureRuntime(e: NodeJS.ProcessEnv) {
  ...
  const storage = createAwsRecordingSegmentStore();
  return { lifecycle: createRecordingLifecycleRepository(database),
    reconcile: createRecordingReconciler(createRecordingReconciliationRepository(database), storage),
    upload: createRecordingSegmentUploader(createRecordingSegmentRepository(database), storage) };
}
