...
type Ledger = { pk: string; jobId: string; fingerprint: string; createdAt: string; expiresAt: number };
export class LabRequestError extends Error {
  constructor(public code: 'lab_request_not_found' | 'lab_request_gone' | 'lab_request_conflict' | 'lab_request_invalid', public statusCode: number) { super(code); }
}
