...
export async function handler(event:ApiGatewayV2Event){
  ...
  api??=createRecordingCleanupReviewApi({configuration:{workforceIssuer:e.WORKFORCE_ISSUER??'',workforceAudience:e.WORKFORCE_AUDIENCE??'',
    organizationId:e.RECORDING_ORGANIZATION_ID??'',phiAllowed:e.PHI_ALLOWED==='true',activation:qualificationActivation(e.RECORDING_CLEANUP_REVIEW_ACTIVATION),...qualificationFrom(e,e.RECORDING_CLEANUP_REVIEW_ACTIVATION),
