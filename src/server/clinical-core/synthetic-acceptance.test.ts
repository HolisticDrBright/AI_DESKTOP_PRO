...
describe("deployed synthetic Cognito-to-Aurora acceptance harness", () => {
  test("runs all eight steps without persisting or printing bearer and invitation tokens", async () => {
    ...
    await expect(runSyntheticApiAcceptance({
      ...
      manifest,
      fetch: fetcher,
    })).resolves.toEqual({ passed: 8, externalRequests: 8 });
    expect(calls).toHaveLength(8);
    expect(calls[0]!.body).not.toHaveProperty("patientName");
    expect(calls[4]!.body).toHaveProperty("patientName", "refused");
  ...
  test("refuses non-API-Gateway origins and malformed or shared ID tokens before any request", async () => {
    ...
    await expect(runSyntheticApiAcceptance({ apiOrigin: "https://example.com", workforceIdToken: token("a"), consumerIdToken: token("b"), manifest, fetch: never }))
      .rejects.toMatchObject({ category: "configuration_invalid" });
    await expect(runSyntheticApiAcceptance({ apiOrigin: "https://abc.execute-api.us-east-2.amazonaws.com", workforceIdToken: token("a"), consumerIdToken: token("a"), manifest, fetch: never }))
      .rejects.toMatchObject({ category: "configuration_invalid" });
  });
});
