...
const requestSchema=z.object({recordingId:uuid,version:z.number().int().positive().safe(),cleanupReleaseId:uuid,workerSha256:sha}).strict();
...
export const recordingCleanupAdmissionSchema=z.object({recordingId:uuid,sessionId:uuid,organizationId:uuid,patientRecordId:uuid,
  ...
  inventory:z.array(segmentSchema).max(4096),inventorySha256:sha,validUntil:z.string().datetime({offset:true}),audioDeleted:z.literal(false)}).strict()
  ...
  return async<T>(context:ProductionClinicalRequestContext,input:unknown,
    ...
    try{
      return await database.transaction(async tx=>{
        ...
        const result=await tx.query<{data:unknown}>('select clinical_private.admit_recording_cleanup($1,$2::bigint,$3,$4) as data',[
        ...
        if(a.recordingId!==r.recordingId||a.organizationId!==context.organizationId||a.version!==r.version
          ||a.cleanupReleaseId!==r.cleanupReleaseId||a.workerSha256!==r.workerSha256)throw new RecordingCleanupError('service_unavailable');
