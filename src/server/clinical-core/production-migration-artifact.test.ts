...
describe('isolated canonical production migration artifact', () => {
  it('produces identical complete manifests and SQL from concurrent builders', () => {
    ...
    expect(parallel[0].manifest.migrations).toHaveLength(86);
    expect(Object.keys(parallel[0].files)).toHaveLength(86);
    expect(parallel[0].manifest.migrations.at(-1)?.file).toBe('20260920040000_production_recording_drafting.sql');
  });
