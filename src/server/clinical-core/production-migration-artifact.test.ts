...
describe('isolated canonical production migration artifact', () => {
  it('produces identical complete manifests and SQL from concurrent builders', () => {
    ...
    expect(parallel[0].manifest.migrations).toHaveLength(70);
    expect(Object.keys(parallel[0].files)).toHaveLength(70);
    expect(parallel[0].manifest.migrations.at(-1)?.file).toBe('20260917130000_production_recording_readiness.sql');
  });
