...
const mock=vi.hoisted(()=>({db:vi.fn(),sfn:vi.fn(),s3:vi.fn(),sign:vi.fn()}));
...
vi.mock('@aws-sdk/client-s3',()=>{class Command{constructor(public input:Record<string,unknown>){}}return {S3Client:class{send=mock.s3;},PutObjectCommand:class extends Command{},HeadObjectCommand:class extends Command{},ListObjectsV2Command:class extends Command{}};});
...
describe('independent production lab processing',()=>{
  ...
  it('blocked deployment refuses every route before identity, consent, billing or storage access',async()=>{
    ...
    for(const request of [event('POST','requests/saved',planInput()),event('GET','inventory'),event('GET',`jobs/${uuid}`),event('POST',`jobs/${uuid}/cancel`,{confirmRemoveUnfinishedAnalysis:true})]){
