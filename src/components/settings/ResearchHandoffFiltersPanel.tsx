            ...
            <ul className="mt-1 flex flex-col gap-0.5 pl-4">
              ...
              {result.posture && (
                <li data-testid="prh-preview-posture">
                  Server posture: transport=<code>{result.posture.transport}</code> · API=
                  <code>{result.posture.clinical_api_id ?? "(unknown)"}</code> · edition=
                  <code>{result.posture.app_edition ?? "(unset)"}</code> · live_mode=
                  <code>{String(result.posture.live_mode)}</code>
                  {result.posture.transport !== "aws_api_gateway" && (
                    <span className="ml-2 rounded border border-critical/25 bg-critical-tint px-2 py-0.5 text-critical-deep">
                      Warning: response did not come from the approved AWS clinical endpoint. The batch
                      IDs shown above are not durable clinical records.
                    </span>
                  )}
                </li>
              )}
