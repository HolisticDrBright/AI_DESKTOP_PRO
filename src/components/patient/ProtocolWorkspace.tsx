...
export function ProtocolWorkspace({ patientId }: { patientId: string }) {
  ...
  return (
    <div data-testid="protocol-workspace" className="flex flex-col gap-3 pt-4">
      ...
      {data.consumerGenerated && <ConsumerGeneratedPlan plan={data.consumerGenerated} />}
...
function ConsumerGeneratedPlan({ plan }: { plan: NonNullable<LivePatientProtocol["consumerGenerated"]> }) {
  return (
    <Card className="p-4" data-testid="consumer-generated-plan">
      <CardTitle>Patient app plan — v{plan.version}</CardTitle>
      <p className="m-0 mt-1 text-[11.5px] leading-[1.5] text-faint">
        Generated by ALP from analyzed labs and patient-reported symptoms · confidence {plan.confidence} · received {fmtDateTime(plan.receivedAt)}
      </p>
      <p className="m-0 mt-3 text-[12.5px] leading-[1.55] text-body">{plan.summary}</p>
      <ul className="m-0 mt-3 flex list-none flex-col gap-2 p-0">
        {plan.tasks.map((task) => (
          <li key={task.id} className="rounded-xl border border-line bg-surface-subtle px-3 py-2 text-[12px] text-body">
            <span className="font-semibold">{task.name}</span>
            <span className="block text-subtle">{task.frequency}{task.timing ? ` · ${task.timing}` : ""}</span>
            {task.notes && <span className="block text-faint">{task.notes}</span>}
          </li>
        ))}
      </ul>
      <p className="m-0 mt-3 text-[11px] leading-[1.5] text-warning-deep">
        No product was selected automatically because the governed product catalog is awaiting approval. This app-generated wellness plan is separate from a practitioner-authored protocol.
      </p>
    </Card>
  );
}
