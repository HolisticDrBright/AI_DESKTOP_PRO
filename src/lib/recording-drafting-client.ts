...
import { draftingOperationSchema, parseDraftingResponse, type DraftingOperation } from '@/contracts/encounterRecordingDrafting';
...
export async function requestRecordingDrafting(request: DraftingOperation, signal: AbortSignal) {
  const parsed = draftingOperationSchema.safeParse(request);
  if (!parsed.success) throw new AdapterError('invalid');
  const requestSignal = AbortSignal.any([signal, AbortSignal.timeout(45000)]);
  try {
    const response = await fetch('/api/live/scribe/drafting', { method: 'POST', credentials: 'same-origin', cache: 'no-store',
